...
隐藏模式会根据唯一模型前缀定位 `{前缀}-STR` 节点及其后代，并将其与搜索命中对象合并为最终保留集合。只有所有条件均为“已找到”，且范围、模型前缀、最终保留集合、宿主实际选择与最终保留集合完全一致、用户确认都有效时，才执行隐藏未选中；完成后恢复最终保留集合为当前选择。
...
8. 任一条件为 NotFound、Duplicate 或 ConditionInvalid 都会阻止隐藏；只有所有条件唯一匹配时，隐藏模式才会提供隐藏确认。
