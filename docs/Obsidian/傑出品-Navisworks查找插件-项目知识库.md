...
多个条件会分别执行搜索，唯一找到项固定进入有效对象集合，重复项由用户逐条决定是否贡献对象，最终集合再按对象去重。因此，当前多条件语义是独立条件的 OR：条件 A 或条件 B 命中即可成为候选；它不是把多个条件组合成一个“同时满足”的 AND 查询。结果状态还要求条件与对象形成一对一关系：同一对象第一次被单值条件命中时有效，后续条件再次指向该对象时标记为跨条件重复。
...
条件级状态严格只有四种：匹配 0 个为“未找到”；匹配 1 个且对象尚未被前序条件占用为“已找到”；单条条件匹配 2 个及以上，或后续条件再次指向同一对象，均为“重复”；条件无法构造或执行为“条件异常”。结果页支持全部、问题项、已找到、未找到、重复和条件异常筛选。“选择”只控制导出，“计入匹配”控制重复项是否进入有效集合；重复项默认不计入。双击重复行会选中对应对象并聚焦相机，查看后可通过“还原全部选择”恢复当前有效集合。
...
**做法：** 先搜索并展示统计。任意未找到、重复或条件异常都会暂停隐藏并要求用户在“仍然继续隐藏 / 返回检查”之间明确选择；自动弹窗和主界面“隐藏未选中”按钮共用同一执行流程。选择继续只覆盖唯一性要求，实际保留对象由“计入匹配”状态决定，未勾选的重复项不贡献对象。纯 `SelectionEquivalencePolicy` 仍以集合语义比较最终保留集合与宿主 CurrentSelection 快照，任何缺失或额外对象都会阻断隐藏。
...
| 单条条件命中多个对象或多条条件指向同一对象仍被当作成功 | 只判断“是否有命中”，没有验证一对一关系 | 将 0、1、2+ 和执行失败建模为四态，并把后续重复引用同一对象的条件标记为“重复” | 值比较、单条件基数和跨条件对象占用是三条独立约束 |
| 发现任意问题项后仍可能进入隐藏流程 | 旧门禁只关注总命中或部分失败 | 将未找到、重复、条件异常全部纳入隐藏前置不变量 | 高影响操作应以所有条件都合法为门槛，而不是以部分成功为门槛 |
| 搜索执行中结果标签已切换但页面仍残留选项内容 | UI 线程执行模型操作期间提前切换选项卡，原生消息泵只能完成部分重绘 | 搜索期间锁定选项卡和关闭操作，模型操作结束后一次性切换并重绘结果页 | 宿主内同步长操作应避免在中间状态切换复杂视图 |
...
- 输入条件非空。
- 搜索范围明确且属于单一模型。
- 每条条件都恰好匹配 1 个未被前序条件占用的对象；未找到、重复和条件异常任一出现即停止隐藏。
- 最终保留集合非空。
...
| 搜索范围 | 必须来自当前选择树快照 |
| 模型要求 | 一次只能识别一个模型前缀 |
| 条件结果 | 0 为未找到；1 且对象未被占用为已找到；2+ 或后续条件重复引用同一对象为重复；无法执行为条件异常 |
