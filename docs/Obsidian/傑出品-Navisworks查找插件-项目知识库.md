...
条件快照会分别保留分类与属性的显示标识和内部标识；CSV、普通日志与可选诊断日志都输出这四个原始字段，而不是只输出回退后的名称。
...
**做法：** 先搜索并写入选择，再展示统计并要求用户确认；任意未找到、重复或条件异常都强制阻断隐藏，同时继续检查最终保留集合和实际选择是否为空。纯 `SelectionEquivalencePolicy` 会以集合语义比较请求的最终保留集合与宿主 CurrentSelection 快照，任何缺失或额外对象都会阻断隐藏。
...
**做法：** 结果保存条件快照；输入发生变化即清空或标记旧结果失效，并禁用导出和创建选择集。XML 导入先完整解析，再原子替换来源路径、条件和结果状态。CSV、普通日志和可选诊断日志均分别写入分类与属性的显示标识、内部标识、状态、匹配数量与说明；CSV 对逗号、引号和换行做字段转义。
...
| 隐藏保护 | 任一未找到、重复或条件异常，以及空保留集合、空实际选择或实际选择与请求集合不等价均阻断 |
