...
条件级状态严格只有四种：匹配 0 个为“未找到”；匹配 1 个且对象尚未被前序条件占用为“已找到”；单条条件匹配 2 个及以上，或后续条件再次指向同一对象，均为“重复”；条件无法构造或执行为“条件异常”。结果页支持全部、问题项、已找到、未找到、重复和条件异常筛选；双击重复行会选中该条件对应的对象，帮助定位模型值或条件重复问题。
