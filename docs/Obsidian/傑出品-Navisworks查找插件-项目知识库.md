...
多个条件会分别执行搜索，最终对象集合取并集并去重。因此，当前多条件语义是独立条件的 OR：条件 A 或条件 B 命中即可进入对象级结果；它不是把多个条件组合成一个“同时满足”的 AND 查询。结果状态还要求条件与对象形成一对一关系：同一对象第一次被单值条件命中时有效，后续条件再次指向该对象时标记为跨条件重复。
...
条件级状态严格只有四种：匹配 0 个为“未找到”；匹配 1 个且对象尚未被前序条件占用为“已找到”；单条条件匹配 2 个及以上，或后续条件再次指向同一对象，均为“重复”；条件无法构造或执行为“条件异常”。结果页支持全部、问题项、已找到、未找到、重复和条件异常筛选；双击重复行会临时选中该条件对应的对象，关闭提示后恢复定位前的全部选择，避免破坏后续隐藏流程。
