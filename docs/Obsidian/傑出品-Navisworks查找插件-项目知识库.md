...
| 模型状态层 | `SelectionService`、`HideService` | 写入当前选择、创建选择集、隐藏最终保留集合之外的对象 |
...
当前选择是一次操作的临时状态；选择集是可随 Navisworks 文档保存的持久状态。两者用途不同，因此项目保留了独立的“创建选择集”动作，而不是在每次搜索后自动创建。条件被导入、编辑、删除或清空时，缓存结果立即失效，导出、选择集和其他基于旧结果的操作都必须停用直到重新搜索。
...
**做法：** 先搜索并写入选择，再展示统计并要求用户确认；任意未找到、重复或条件异常都强制阻断隐藏，同时继续检查最终保留集合和实际选择是否为空。

**收益：** 即使 XML 错误、范围错误或 API 状态异常，也不会轻易隐藏整个模型。

**不变量：** “隐藏未选中”只能在全部条件均唯一匹配时发生。存在任何问题项时，没有覆盖或继续执行的例外。
...
**做法：** 普通结果日志与可选诊断会话分开，诊断开关由用户显式启用。
...
**做法：** 结果保存条件快照；输入发生变化即清空或标记旧结果失效，并禁用导出和创建选择集。CSV、普通日志和可选诊断日志均写入完整条件、状态、匹配数量与说明；CSV 对逗号、引号和换行做字段转义。
...
项目文件接受显式 `NavisworksInstallDir`，其次读取 `NAVISWORKS_2023_PATH`，最后回退到标准 Program Files 安装目录。主构建脚本遵循相同规则，并将解析后的目录传给 MSBuild；它不再依赖个人安装路径或特定 Build Tools 目录。

仍需在目标机器上确认 Navisworks API DLL、MSBuild 工作负载和插件安装权限可用；这些是宿主环境条件，不是仓库内的硬编码配置。

### 6.2 自动化测试覆盖不足

当前核心流程高度依赖 Navisworks 运行时，代码图索引未发现对应自动化测试。风险集中在：
...
可以先测试纯逻辑，再用安装后的 Navisworks 做集成冒烟验证，不必一开始就模拟完整宿主。
...
当前工作区存在尚未提交的 API 兼容调整，尝试通过运行时探测兼容较旧 Navisworks 成员；但主项目定位仍是 2023 单版本。应先明确这是 2023 的防御性兼容，还是多版本项目的能力，再决定是否保留在主仓库。

## 7. 建议的后续演进顺序

| 优先级 | 建议 | 价值 |
|---|---|---|
| P0 | 按四态矩阵完成 Navisworks 手工验收 | 验证重复选择、严格隐藏门禁、CSV 转义和 DPI 布局 |
...
| 隐藏保护 | 任一未找到、重复或条件异常，以及空保留集合、空实际选择均阻断 |
...
> [!note] 文档依据
> 本文依据 2026-07-13 的当前工作区、CodeGraph 调用关系、项目配置、README、CHANGELOG 和 Git 提交历史整理。文档未复制项目源码，只保留必要的组件名称、行为规则与架构关系。
